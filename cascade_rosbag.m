function [image_list,dets] = cascade_rosbag( rosbag, with_pausing )
num_results = 200;
indices = floor(linspace(1,rosbag.NumMessages,num_results));
load('INRIA/inriaperson_final');
model.interval = 10;
image_list={};
dets={};
for i = 1:num_results
    message = readMessages(rosbag,indices(i));
    image = readImage(message{1});
    [next_annotated_image, next_det] = test(image,model);
    [image_list] = [image_list; image];
    [dets] = [dets; {next_det i}];    
    if with_pausing
        fprintf('Press any key to run next image..');
        pause;
    else
        fprintf('Running next image.. %d of %d',i,num_results);
        pause(0.05);
    end
end
end
